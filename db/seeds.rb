...
#VISITS
location.visits.build(user_name: 'Juanjo', from_date: Time.now, to_date: Time.now + 1.hour, user_id: user1.id)
location.save

visit = Visit.create(user_name: 'Joselu', from_date: Time.now, to_date: Time.now + 1.hour, user_id: user1.id)
visit.location = location2
visit.save

visit2 = Visit.create(user_name: 'JuanRa', from_date: Time.now, to_date: Time.now + 1.hour, user_id: user2.id)
location3.visits << visit2

Visit.create(user_name: 'JuanRa', from_date: Time.now, to_date: Time.now + 1.hour, location_id: location2.id, user_id: user2.id)
